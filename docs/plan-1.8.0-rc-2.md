...
3. Make the signed OTA/browser updater understand the exact RC identifier
   `1.8.0-rc-2` and never mistake it for an older `1.8.0-rc` image.
...
- The release catalog marks `1.8.0-rc-2` as `rc`, not `stable`.
...
- New X4 Pro installations restore the saved frontlight state after wake by
  default.
- An explicit user choice to keep the light off remains respected; brightness
  and warmth continue to be stored independently.
...
- X3/X4, X4 Pro, and Sticky boot and report `1.8.0-rc-2`.
